...
function tfidfSnippet(config) {
  const lines = [
    '# NLP TF-IDF Vectorizer Transform',
    `config = ${pyValue(config)}`,
    '',
    'def apply_tfidf(x):',
    '    """Apply TF-IDF vectorization to text input."""',
    '    return apply_transform("transform.text.tfidf", x, config)',
  ];
  return lines.join('\n');
}

function countVectorizerSnippet(config) {
  const lines = [
    '# NLP Count Vectorizer (Bag of Words) Transform',
    `config = ${pyValue(config)}`,
    '',
    'def apply_count_vectorizer(x):',
    '    """Apply count vectorization to text input."""',
    '    return apply_transform("transform.text.count_vectorizer", x, config)',
  ];
  return lines.join('\n');
}

function embeddingSnippet(config) {
  const lines = [
    '# NLP Dense Text Embedding Transform',
    `config = ${pyValue(config)}`,
    '',
    'def apply_embedding(x):',
    '    """Generate dense vector embeddings for text input."""',
    '    return apply_transform("transform.text.embedding", x, config)',
  ];
  return lines.join('\n');
}

function stemLemmatizeSnippet(config) {
  const lines = [
    '# NLP Stemming & Lemmatization Transform',
    `config = ${pyValue(config)}`,
    '',
    'def apply_stem_lemmatize(x):',
    '    """Apply stemming or lemmatization to text tokens."""',
    '    return apply_transform("transform.text.stem_lemmatize", x, config)',
  ];
  return lines.join('\n');
}
...
export function generateTransformPythonCode(nodeType, config = {}) {
  ...
  if (nodeType === 'transform.text.tfidf') return tfidfSnippet(config);
  if (nodeType === 'transform.text.count_vectorizer') return countVectorizerSnippet(config);
  if (nodeType === 'transform.text.embedding') return embeddingSnippet(config);
  if (nodeType === 'transform.text.stem_lemmatize') return stemLemmatizeSnippet(config);
