    ...
    class NumpyClass:
        ...
        def average(values, axis=None, weights=None):
            ...
            def _avg(column_values):
                if column_values and _is_array_like(column_values[0]):
            ...
            if _is_array_like(first):
                if first and _is_array_like(first[0]):
...
def _lifecycle_training(inputs: Dict[str, Any], config: Dict[str, Any]) -> Any:
    ...
    # The saved model owns CSV preprocessing: categorical values are one-hot encoded
    # and numeric NaNs are median-imputed before the estimator receives features.
