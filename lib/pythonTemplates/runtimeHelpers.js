...
def _lifecycle_split(inputs: Dict[str, Any], config: Dict[str, Any]) -> Any:
    ...
    if isinstance(data, list) and len(data) > 0:
        ...
        return {
            'train': {'data': [data[i] for i in train_idx], 'indices': train_idx.tolist() if hasattr(train_idx, 'tolist') else list(train_idx)},
            'val': {'data': [data[i] for i in val_idx], 'indices': val_idx.tolist() if hasattr(val_idx, 'tolist') else list(val_idx)},
            'test': {'data': [data[i] for i in test_idx], 'indices': test_idx.tolist() if hasattr(test_idx, 'tolist') else list(test_idx)},
        }
