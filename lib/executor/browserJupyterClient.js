...
async function readJsonResponse(response, endpoint) {
  ...
  if (looksLikeHtml) {
    throw new Error(`The Jupyter proxy returned HTML instead of JSON for ${endpoint}. Check the Jupyter URL and access token.`);
  }
  throw new Error(`The Jupyter proxy returned invalid JSON for ${endpoint} (${response.status}, ${contentType}).`);
}
...
export class BrowserJupyterClient {
  ...
  async startKernel({ fresh = false } = {}) {
    ...
    const data = await readJsonResponse(response, '/api/jupyter/kernel');
  ...
  async executeCode(kernelId, code, { username = 'proto-ml-browser', nodeId } = {}) {
    ...
    const data = await readJsonResponse(response, '/api/jupyter/cell');
    if (!response.ok) {
