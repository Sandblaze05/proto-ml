import { previewClientImageUpload, previewClientUpload, previewClientTextUpload } from '../clientUploadStore';
...
import { resolveBrowserPreviewCapability } from './browserExecutionCapabilities.js';

export { resolveBrowserPreviewCapability };

export function getClientPreviewCapability(graph, targetNodeId) {
  return resolveBrowserPreviewCapability(graph, targetNodeId);
}
...
async function runDatasetNode(node, n) {
  ...
  if (node.type === 'dataset.text') {
    if (!config.client_upload_id) {
      throw new Error('Text preview is client-only. Upload files to this node first.');
    }

    return previewClientTextUpload(config.client_upload_id, {
      ...config,
      n,
    });
  }
  ...
  if (node.type === 'dataset.database') {
    try {
      const res = await fetch('/api/datasets/database', {
        ...
        body: JSON.stringify({ action: 'inspect', config: { ...config, limit: n } }),
      });
      if (res.ok) {
        const payload = await res.json();
        if (payload.ok) {
    ...
    const mockRows = Array.isArray(config.mockRows) && config.mockRows.length > 0 ? config.mockRows.slice(0, n) : [{
      _preview: true,
      _warning: 'Database preview returned synthetic sample.',
    ...
    return {
      ...
      features: mockRows,
      targets: [],
      metadata: { db_type: config.db_type, table: config.table },
    };
  }
...
async function runNode(node, context, n) {
  ...
  if (nodeKind === 'lifecycle') {
    throw new Error('Lifecycle nodes perform real model work. Use Cell Run or Run Script instead of synthetic preview.');
  }
