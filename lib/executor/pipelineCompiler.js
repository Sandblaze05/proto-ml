...
function inferDefaultTargetHandle(nodeDef) {
  const requiredInputs = normalizePortDefs(nodeDef?.inputs).filter((port) => port.optional === false);
  if (requiredInputs.length === 1) return requiredInputs[0].name;
  return inferDefaultPortName(nodeDef?.inputs, ['in', 'input', 'dataset', 'data']);
}
