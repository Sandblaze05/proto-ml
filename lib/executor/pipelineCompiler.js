...
import { getNodeDef, getInputPorts, getOutputPorts, getNodeConfigDefaults, getNodeConfigSchema, getNodeCacheMetadata } from '../../nodes/nodeRegistry.js';
import { validateNodeSpec } from '../runtimeSpec/nodeSpec.js';
import compilerDispatch from '../runtimeSpec/compilerDispatch.js';
import { computeNodeHash, isDeterministic, getSeed, getCacheKey } from '../runtimeSpec/cacheMetadata.js';
...
function artifactToPythonLiteral(artifact) {
  if (!artifact || typeof artifact !== 'object') return 'None';
  return `{
    'datatype': ${toPythonLiteral(artifact.datatype || 'tabular')},
    'schema': ${toPythonLiteral(artifact.schema || {})},
    'row_count': ${toPythonLiteral(artifact.rowCount ?? null)},
    'shape': ${toPythonLiteral(artifact.shape ?? [])},
    'serialization_format': ${toPythonLiteral(artifact.serializationFormat || 'json')},
    'materialization_ref': ${toPythonLiteral(artifact.materializationRef || null)},
    'lineage_node_id': ${toPythonLiteral(artifact.lineage?.nodeId || null)},
    'lineage_port_name': ${toPythonLiteral(artifact.lineage?.portName || null)},
    'lineage_produced_at': ${toPythonLiteral(artifact.lineage?.producedAt || null)},
  }`;
}
...
function normalizeNodeRecord(node = {}) {
  const model = node?.data?.nodeModel || node?.nodeModel || {};
  const type = model.type || node.type || 'unknown';
  const config = model.config || model.params || node.config || {};
  const inputs = Array.isArray(model.inputs) ? model.inputs : (Array.isArray(node.inputs) ? node.inputs : []);
  const outputs = Array.isArray(model.outputs) ? model.outputs : (Array.isArray(node.outputs) ? node.outputs : []);

  return {
    ...node,
    type,
    config,
    pythonCode: model.pythonCode || model.execution_code || node.pythonCode || '',
    label: model.label || node.label || type,
    inputs,
    outputs,
    kind: model.kind || node.kind || getNodeDef(type)?.kind || 'generic',
    category: model.category || node.category || null,
    metadata: model.metadata || node.metadata || {},
    accepts: model.accepts || node.accepts || [],
    produces: model.produces || node.produces || [],
    uiSchema: model.uiSchema || node.uiSchema || {},
  };
}
...
function inferDefaultSourceHandle(nodeType) {
  return inferDefaultPortName(getOutputPorts(nodeType), ['out', 'data', 'rows', 'merged']);
}

function inferDefaultTargetHandle(nodeType) {
  const allInputs = normalizePortDefs(getInputPorts(nodeType));
  if (allInputs.length === 1) return allInputs[0].name;
  return null;
}

function getEdgeTargetHandle(edge, targetNodeType) {
  if (edge?.targetHandle && typeof edge.targetHandle === 'string') return edge.targetHandle;
  return inferDefaultTargetHandle(targetNodeType);
}

function collectProvidedInputHandles(inEdges = [], targetNodeType) {
  const handles = new Set();
  let hasUnnamedInput = false;

  for (const edge of inEdges) {
    const inferredHandle = getEdgeTargetHandle(edge, targetNodeType);
    if (inferredHandle) {
...
function validateGraphSemantics({ nodesById, order, incoming, mode }) {
  ...
  for (const nodeId of order) {
    ...
    const sourceOutputs = normalizePortDefs(getOutputPorts(sourceType));
    ...
    for (const edge of outEdges) {
      ...
      const targetInputs = normalizePortDefs(getInputPorts(targetType));
      ...
      if (sourceDef) {
        if (edge.sourceHandle) {
          ...
          const inferredSourceHandle = inferDefaultSourceHandle(sourceType);
      ...
      if (targetDef) {
        if (edge.targetHandle) {
          ...
          const inferredTargetHandle = inferDefaultTargetHandle(targetType);
  ...
  for (const nodeId of order) {
    ...
    if ((stage === 'transform' || stage === 'lifecycle') && inEdges.length === 0 && nodeType !== 'lifecycle.core.model_builder') {
      pushIssue(createValidationMessage(nodeId, nodeType, 'Node has no upstream input.'));
    }

    const requiredInputs = getInputPorts(nodeType).filter((port) => port && port.optional === false);
    if (requiredInputs.length === 0) continue;

    const { handles: providedHandles, hasUnnamedInput } = collectProvidedInputHandles(inEdges, nodeType);
...
function classifyNode(nodeType = '') {
  return compilerDispatch.classify(nodeType);
}
...
function mapLifecycleDispatchStage(nodeType = '') {
  return compilerDispatch.mapLifecycleStage(nodeType);
}
...
export function compileExecutionGraph({ nodes = {}, edges = [] } = {}, options = {}) {
  ...
  const nodesById = Array.isArray(nodes)
    ? Object.fromEntries(nodes
      ...
      .map((n) => [n.id, normalizeNodeRecord(n)]))
    : Object.fromEntries(Object.entries(nodes || {})
      .map(([id, node]) => [id, normalizeNodeRecord({ ...(node || {}), id })]));
  ...
  const outgoingCounts = Object.fromEntries(nodeIds.map((id) => [id, 0]));
  normalizedEdges.forEach((edge) => {
    if (outgoingCounts[edge.source] !== undefined) {
      outgoingCounts[edge.source] += 1;
    }
  });
  const leafNodes = order.filter((id) => outgoingCounts[id] === 0);
  ...
  for (const nodeId of order) {
    ...
    if (stage === 'dataset') {
      const outputHandles = normalizePortDefs(getOutputPorts(nodeType))
        .map((port) => port.name)
        .filter(Boolean);
  ...
  sectionBody.push(`    'artifacts': {node_id: node_outputs.get(node_id) for node_id in graph_spec.get('leafNodes', [])},`);
  sectionBody.push('}');
  ...
  const code = [
    ...
    '    leaf_nodes = graph_spec.get("leafNodes", [])',
    '    selected_node_id = None',
    '    selected_output = None',
    '    for _node_id in reversed(leaf_nodes):',
    '        _candidate = node_outputs.get(_node_id)',
    '        if isinstance(_candidate, dict) and "out" in _candidate:',
    '            _candidate = _candidate.get("out")',
    '        if isinstance(_candidate, list):',
    '            selected_node_id = _node_id',
    '            selected_output = _candidate',
    '            break',
    '    if selected_output is None and len(leaf_nodes) > 0:',
    '        _fallback_id = leaf_nodes[-1]',
    '        _fallback_value = node_outputs.get(_fallback_id)',
    '        if isinstance(_fallback_value, dict) and "out" in _fallback_value:',
    '            _fallback_value = _fallback_value.get("out")',
    '        selected_node_id = _fallback_id',
    '        selected_output = _fallback_value',
    '    final_payload = {',
    '        "graph_spec": graph_spec,',
    '        "final_node_id": selected_node_id,',
    '        "final_output": selected_output,',
    '        "node_outputs": node_outputs,',
    '        "leaf_outputs": {k: node_outputs.get(k) for k in leaf_nodes},',
    '    }',
