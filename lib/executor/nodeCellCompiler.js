 ...
 */
export function compileBootstrapCell(variables = {}) {
  const runtimeCode = getPythonRuntimeCode();
  
  const varAssignments = Object.entries(variables || {})
    .map(([name, value]) => `${name} = ${toPythonLiteral(value)}`)
    .join('\n');
...
export function compileNodeCell(node, inEdges = []) {
  ...
  if (stage === 'dataset') {
    lines.push(
      `${outputVar} = _materialize_dataset({'dataset_type': ${toPythonLiteral(nodeType)}, 'config': _pml_node_cfg})`,
    );
  }
