...
const exactFactories = {
  ...
  'dataset.database': (config) => new DatabaseDatasetRuntime(config),
  'dataset.api': (config) => new APIDatasetRuntime(config),
  'jupyter.execute': (config) => new JupyterNodeRuntime(config),
};
