...
class LifecyclePreviewRuntime {
  ...
  _split(input) {
    ...
    let trainCount = Math.max(0, Math.min(total, Math.floor((trainPct / 100) * total)));
    let valCount = Math.max(0, Math.min(total - trainCount, Math.floor((valPct / 100) * total)));
    if (total > 0 && trainCount === 0) {
      trainCount = 1;
      valCount = Math.min(valCount, Math.max(0, total - trainCount));
    }
    return DataArtifact.wrap({
