...
export async function listClientUploads() {
  ...
  return records
    ...
    .map((entry) => ({
      ...
      clientOnly: true,
      sourceType: entry.metadata?.sourceType || (entry.files.some((f) => f.csv) ? 'csv' : (entry.files.some((f) => f.json || isJsonFileName(f.name)) ? 'json' : (entry.files.some((f) => isImageFileName(f.name)) ? 'image' : 'generic'))),
