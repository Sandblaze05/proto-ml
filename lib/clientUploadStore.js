...
const VERY_LARGE_UPLOAD_BYTES = 250 * 1024 * 1024;

export function getUploadPerformanceAdvice(totalBytes = 0) {
  const bytes = Number(totalBytes) || 0;
  if (bytes >= VERY_LARGE_UPLOAD_BYTES) {
    return [
      'Very large upload: keep the source in object storage or a Jupyter/Kaggle environment and use a URI instead of loading it into the browser.',
      'Use a sampled preview before running the full pipeline.',
      'Prefer Jupyter or another remote runtime for full execution; browser memory is limited.',
    ];
  }
  if (bytes >= LARGE_UPLOAD_BYTES) {
    return [
      'Large upload: previews use a small sample to keep the interface responsive.',
      'Run preprocessing remotely when possible and avoid sending the full dataset through the browser.',
    ];
  }
  return [];
}
...
async function buildTextMetadata(entry, options = {}) {
  const hasCsv = entry.files.some((f) => f.csv || getFileExtension(f.name) === '.csv');
  const hasJsonl = entry.files.some((f) => getFileExtension(f.name) === '.jsonl');
  const fileFormat = String(
    options.file_format || (hasCsv ? 'csv' : (hasJsonl ? 'jsonl' : 'txt'))
  ).toLowerCase();

  const textFiles = entry.files.filter((item) => {
    const ext = getFileExtension(item.name);
    if (fileFormat === 'csv') return item.csv || ext === '.csv';
    if (fileFormat === 'jsonl') return ext === '.jsonl';
    return isTextFileName(item.name) || ext === '.txt' || ext === '.md' || ext === '.csv' || ext === '.jsonl';
  });
  ...
  const maxRecords = Number.isFinite(Number(options.n))
    ? Math.max(1, Number(options.n))
  ...
  for (const item of textFiles) {
    ...
    try {
      ...
      const isCsv = fileFormat === 'csv' || getFileExtension(item.name) === '.csv';
      const parsed = isCsv
        ? parseCsvText(text, options.delimiter || ',', options.header !== false, maxRecords)
        : null;
      ...
      if (parsed && parsed.rows.length > 0) {
        ...
        fieldnames.forEach((fn) => detectedColsSet.add(fn));

        // Resolve text column flexibly
        let textKey = options.text_column || 'text';
        if (!(textKey in firstRow)) {
          const lowerMap = Object.fromEntries(fieldnames.map((fn) => [fn.toLowerCase(), fn]));
          const candidates = [
            String(options.text_column || '').toLowerCase(),
            'text', 'sentence', 'review', 'content', 'message', 'document', 'body', 'input', 'comment', 'description'
          ].filter(Boolean);
          for (const cand of candidates) {
            if (cand in lowerMap) {
              textKey = lowerMap[cand];
              break;
            }
          }
          if (!(textKey in firstRow)) {
            const fallbackKey = fieldnames.find((fn) => !fn.toLowerCase().startsWith('label') && !fn.toLowerCase().startsWith('target') && !fn.toLowerCase().startsWith('id') && !fn.toLowerCase().startsWith('class') && typeof firstRow[fn] === 'string' && firstRow[fn].trim().length > 0);
            if (fallbackKey) textKey = fallbackKey;
          }
        }

        // Resolve label column flexibly
        let labelKey = options.label_column || 'label';
        if (!(labelKey in firstRow)) {
          const lowerMap = Object.fromEntries(fieldnames.map((fn) => [fn.toLowerCase(), fn]));
          const candidates = [
            String(options.label_column || '').toLowerCase(),
            'label', 'sentiment', 'category', 'target', 'class', 'y'
          ].filter(Boolean);
          for (const cand of candidates) {
            if (cand in lowerMap) {
              labelKey = lowerMap[cand];
              break;
            }
          }
        }
        ...
        rows = parsed.rows.map((row) => ({
          ...row,
          text: row[textKey] ?? row.text ?? row.col_0 ?? '',
          label: row[labelKey] ?? row.label ?? row.col_1 ?? null,
        }));
      } else {
        rows = text.split(/\r?\n/).map((line) => ({ text: line.trim(), label: null })).filter((row) => row.text);
      }
      ...
      rows.forEach((row, idx) => {
        ...
        tokens.forEach((t) => {
          if (vocabSet.size < 10000) vocabSet.add(t);
        });
      });
    } catch {
      // Continue if reading a single file fails
    }
  }

  const columns = Array.from(new Set(['text', 'label', ...detectedColsSet, '_source', '_line']));
  const avgLineLength = records.length > 0 ? Math.round(totalWords / records.length) : 0;
  ...
  return {
    ...
    schema: {
      primary: null,
      columns: [
        { name: 'text', dtype: 'string', kind: 'text' },
        { name: 'label', dtype: 'string', kind: 'categorical' },
        { name: '_source', dtype: 'string', kind: 'categorical' },
        { name: '_line', dtype: 'number', kind: 'numeric' },
      ],
      columnTypes: {
        text: 'string',
        label: 'string',
    ...
    records,
    rows: records,
    sampleRows: records.slice(0, sampleRows),
    preview: records.slice(0, previewRows),
    metadata: {
      datasetId: entry.id,
      sourceType: 'text',
      rows: records.length,
...
const SESSION_UPLOAD_PREFIX = 'proto_ml_session_upload_';
const SESSION_UPLOAD_INDEX_KEY = 'proto_ml_session_upload_ids';

function getSessionUploadIds() {
  if (!isSessionStorageAvailable()) return [];
  try {
    const raw = window.sessionStorage.getItem(SESSION_UPLOAD_INDEX_KEY);
...
function saveUploadRecordToSession(record) {
  if (!isSessionStorageAvailable() || !record || !record.id) return;
  try {
    const serializable = {
      ...record,
    ...
    window.sessionStorage.setItem(`${SESSION_UPLOAD_PREFIX}${record.id}`, JSON.stringify(serializable));
    const ids = new Set(getSessionUploadIds());
    ids.add(record.id);
    window.sessionStorage.setItem(SESSION_UPLOAD_INDEX_KEY, JSON.stringify(Array.from(ids)));
  } catch {
    // Quota reached or storage restricted
  }
}

function getUploadRecordFromSession(uploadId) {
  if (!isSessionStorageAvailable() || !uploadId) return null;
  try {
    const raw = window.sessionStorage.getItem(`${SESSION_UPLOAD_PREFIX}${uploadId}`);
...
function getAllUploadRecordsFromSession() {
  if (!isSessionStorageAvailable()) return [];
  const ids = getSessionUploadIds();
  const records = [];
  for (const id of ids) {
    const rec = getUploadRecordFromSession(id);
    if (rec) records.push(rec);
  }
  return records;
}

function removeSessionUploadRecord(uploadId) {
  if (!isSessionStorageAvailable() || !uploadId) return;
  try {
    const ids = getSessionUploadIds().filter((id) => id !== uploadId);
    window.sessionStorage.setItem(SESSION_UPLOAD_INDEX_KEY, JSON.stringify(ids));
    window.sessionStorage.removeItem(`${SESSION_UPLOAD_PREFIX}${uploadId}`);
  } catch {
    // Ignore error
  }
}
...
async function putUploadRecord(record) {
  saveUploadRecordToSession(record);
  try {
    ...
    // Storage fallback to sessionStorage handled by saveUploadRecordToSession
  }
}

async function deleteUploadRecord(uploadId) {
  removeSessionUploadRecord(uploadId);
  try {
    const db = await openDb();
    const tx = db.transaction(STORE_UPLOADS, 'readwrite');
    const store = tx.objectStore(STORE_UPLOADS);
...
async function getUploadRecord(uploadId) {
  const fromSession = getUploadRecordFromSession(uploadId);
...
async function getAllUploadRecords() {
  const sessionRecords = getAllUploadRecordsFromSession();
  let idbRecords = [];
  try {
    const db = await openDb();
    const tx = db.transaction(STORE_UPLOADS, 'readonly');
    ...
    db.close();
    if (Array.isArray(result)) idbRecords = result;
  } catch {
    // Ignore IDB errors
  }
  const byId = new Map();
  idbRecords.forEach((r) => byId.set(r.id, r));
...
async function getUploadEntry(uploadId) {
  ...
  const fromSession = getUploadRecordFromSession(uploadId);
  if (fromSession) {
    uploadStore.set(uploadId, fromSession);
    return fromSession;
  }
...
export async function createClientUpload(files = []) {
  ...
  const normalized = files
    ...
    .map((file) => ({
      ...
      json: isJsonFileName(file.webkitRelativePath || file.name),
      text: isTextFileName(file.webkitRelativePath || file.name),
  ...
  try {
    ...
    const analysis = largeUpload
      ? null
      : await inspectClientUpload(id, { sampleRows: 100, previewRows: 100, n: 100000 });
  ...
  return {
    ...
    jsonFiles: normalized.filter((f) => f.json).map((f) => f.name),
    totalBytes,
    sessionOnly: record.sessionOnly,
    warning,
    recommendations: getUploadPerformanceAdvice(totalBytes),
...
export async function listClientUploads() {
  ...
  return records
    ...
    .map((entry) => ({
      ...
      clientOnly: true,
      sourceType: entry.metadata?.sourceType || (entry.files.some((f) => f.csv) ? 'csv' : (entry.files.some((f) => f.json || isJsonFileName(f.name)) ? 'json' : (entry.files.some((f) => f.text || isTextFileName(f.name)) ? 'text' : (entry.files.some((f) => isImageFileName(f.name)) ? 'image' : 'generic')))),
...
function flattenJsonValue(value, prefix = '', out = {}) {
  ...
  if (value && typeof value === 'object') {
    Object.entries(value).forEach(([key, nested]) => {
      ...
      flattenJsonValue(nested, nextPrefix, out);
    });
    return out;
  }
...
function flattenJsonRow(row) {
  if (row && typeof row === 'object' && !Array.isArray(row)) return flattenJsonValue(row);
  return { value: row };
}

function parseJsonText(raw, options = {}) {
  const format = String(options.file_format || 'json').toLowerCase();
  const dataKey = options.data_key || '';
  const rows = [];
  ...
  if (format === 'jsonl') {
    String(raw).split(/\r?\n/).forEach((line, idx) => {
      ...
      if (!trimmed) return;
      try {
        rows.push(flattenJsonRow(JSON.parse(trimmed)));
      } catch {
    ...
    const extracted = getByPath(payload, dataKey);
    if (Array.isArray(extracted)) {
      rows.push(...extracted.map(flattenJsonRow));
    } else if (!dataKey && payload && typeof payload === 'object' && Array.isArray(payload.data)) {
      rows.push(...payload.data.map(flattenJsonRow));
    } else if (extracted && typeof extracted === 'object') {
      rows.push(flattenJsonRow(extracted));
    } else {
      throw new Error('JSON data_key must resolve to an object or array');
    }
  }
...
async function getJsonTables(uploadId, options = {}) {
  const entry = await getUploadEntry(uploadId);
  if (!entry) throw new Error('Client upload not found');

  const fileFormat = String(options.file_format || 'json').toLowerCase();
  const expectedExt = fileFormat === 'jsonl' ? '.jsonl' : '.json';
  const jsonFiles = entry.files.filter((f) => f.json && getFileExtension(f.name) === expectedExt);
  if (jsonFiles.length === 0) throw new Error('No JSON files found in uploaded client files');

  const tables = {};
  for (const item of jsonFiles) {
    const parsed = parseJsonText(await item.file.text(), options);
    const analysis = profileColumns(parsed.rows, parsed.columns);
    tables[getFileStem(item.name)] = {
      file: item.name,
      columns: parsed.columns,
      rows: parsed.rows,
      sampled: false,
      sampledBytes: Number(item.size || 0),
...
async function getCsvTables(uploadId, delimiter, header, sampleBytes = CSV_SAMPLE_BYTES) {
  const entry = await getUploadEntry(uploadId);
  if (!entry) throw new Error('Client upload not found');
  ...
  for (const item of csvFiles) {
    const sample = await readCsvSampleText(item.file, sampleBytes);
...
export async function inspectClientUpload(uploadId, options = {}) {
  ...
  const csvFiles = entry.files.filter((f) => f.csv);
  const jsonFiles = entry.files.filter((f) => f.json || isJsonFileName(f.name));
  const textFiles = entry.files.filter((f) => f.text || isTextFileName(f.name));
  const imageFiles = entry.files.filter((f) => isImageFileName(f.name));
  const sampleBytes = Number.isFinite(Number(options.sampleBytes))
    ? Math.max(64 * 1024, Number(options.sampleBytes))
    : (entry.totalBytes >= LARGE_UPLOAD_BYTES ? 1024 * 1024 : CSV_SAMPLE_BYTES);

  if (csvFiles.length > 0) {
    const tables = await getCsvTables(uploadId, delimiter, header, sampleBytes);
  ...
  if (jsonFiles.length > 0) {
    const tables = await getJsonTables(uploadId, options);
    const tableNames = Object.keys(tables);
    const primary = options.primary && tables[options.primary] ? options.primary : tableNames[0];
    const primaryTable = tables[primary];
    const analysis = profileColumns(primaryTable.rows, primaryTable.columns);
    ...
    const metadata = buildDatasetMetadata({
      ...
      features: Array.isArray(options.features)
        ? options.features
        : (Array.isArray(options.feature_columns) ? options.feature_columns : (Array.isArray(options.feature_keys) ? options.feature_keys : [])),
      missingStrategy: (options.missing && options.missing.strategy) || options.handle_missing || 'drop',
      totalBytes: entry.totalBytes,
      sampled: false,
      sampledBytes: primaryTable.sampledBytes,
  ...
  if (textFiles.length > 0) {
    return buildTextMetadata(entry, options);
  }
...
export async function previewClientUpload(uploadId, options = {}) {
  ...
  const n = Number.isFinite(Number(options.n)) ? Math.max(1, Number(options.n)) : 5;
  const delimiter = options.delimiter || ',';
  const header = options.header !== false;
  const relations = Array.isArray(options.relations) ? options.relations : [];
  const targetColumn = options.target_column || '';
  ...
  const isTextUpload = entry.files.some((f) => f.text || isTextFileName(f.name)) && !entry.files.some((f) => f.csv);
  if (isTextUpload) {
    return previewClientTextUpload(uploadId, options);
  }

  const isJsonUpload = entry.files.some((f) => f.json || isJsonFileName(f.name)) && !entry.files.some((f) => f.csv);

  const tables = isJsonUpload
    ? await getJsonTables(uploadId, options)
    : await getCsvTables(uploadId, delimiter, header);
  const tableNames = Object.keys(tables);
  const primary = options.primary && tables[options.primary] ? options.primary : tableNames[0];
  ...
  return {
    ...
    metadata: {
      ...
      tables: tableNames,
      target: targetColumn || null,
      sourceType: isJsonUpload ? 'json' : 'csv',
...
export async function previewClientTextUpload(uploadId, options = {}) {
  const entry = await getUploadEntry(uploadId);
  if (!entry) throw new Error('Client upload not found');

  const analysis = await buildTextMetadata(entry, options);
  const allRows = analysis.records || analysis.rows || analysis.sampleRows || [];
  return {
    ...analysis,
    type: 'text',
    count: analysis.stats?.rows || allRows.length,
    root: `client://${entry.id}`,
    records: allRows,
    rows: allRows,
    sampleRows: analysis.sampleRows || allRows.slice(0, 50),
    vocab_size: analysis.stats?.vocabSize || 0,
    clientOnly: true,
  };
}
