...
const VALID_PORT_ROLES = Object.freeze([
  ...
  'config', 'labels', 'targets', 'unknown',
]);

const VALID_DATATYPES = Object.freeze([
  'tabular', 'tensor', 'sequence', 'dataloader', 'list', 'dict',
  'model', 'loss', 'metrics', 'text', 'image', 'any',
]);
...
function assertValidPort(port, context) {
  ...
  if (!VALID_DATATYPES.includes(port.datatype.trim())) {
    throw new Error(
      `Invalid port in ${context}: datatype '${port.datatype}' is not in the valid set: ${VALID_DATATYPES.join(', ')}.`,
    );
  }
}
...
export function getPortContract(nodeSpec) {
  return {
    inputs: nodeSpec.ports.inputs.map((p) => ({
      ...
      artifact: {
        schema: p.artifact?.schema ?? null,
        serializationFormat: p.artifact?.serializationFormat ?? null,
        materializationRef: p.artifact?.materializationRef ?? null,
      },
    })),
