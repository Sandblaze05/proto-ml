import { getNodeDef, getInputPorts, getOutputPorts, getNodeConfigDefaults, getNodeConfigSchema } from '../../nodes/nodeRegistry.js';
...
function inferKind(nodeType = '', nodeDef) {
  ...
  const registeredKind = getNodeDef(nodeType)?.kind;
...
export function buildCanvasPayloadFromTemplateGraph(graph, options = {}) {
  ...
  nodes.forEach((node, index) => {
    ...
    const inputs = getInputPorts(node.type).map((port) => port.name);
    const outputs = getOutputPorts(node.type).map((port) => port.name);
    const config = { ...(getNodeConfigDefaults(node.type) || {}), ...(node.config || {}) };
    ...
    const nodeModel = {
      ...
      config,
      params: config,
      uiSchema: { ...(getNodeConfigSchema(node.type) || {}) },
      accepts: def?.accepts || [],
