...
var ShaderMount = class {
  ...
  constructor(parentElement, fragmentShader, uniforms, webGlContextAttributes, speed = 0, frame = 0, minPixelRatio = 2, maxPixelCount = DEFAULT_MAX_PIXEL_COUNT, mipmaps = []) {
    if (typeof HTMLElement !== "undefined" && parentElement instanceof HTMLElement) {
  ...
  handleResize = () => {
    ...
    const pinchZoom = (typeof visualViewport !== "undefined" ? visualViewport?.scale : 1) ?? 1;
...
function bestGuessBrowserZoom() {
  ...
  const viewportScale = (typeof visualViewport !== "undefined" ? visualViewport?.scale : 1) ?? 1;
  const viewportWidth = (typeof visualViewport !== "undefined" ? visualViewport?.width : window.innerWidth) ?? window.innerWidth;
  const scrollbarWidth = window.innerWidth - (document.documentElement?.clientWidth || window.innerWidth);
  const innerWidth = viewportScale * viewportWidth + scrollbarWidth;
  const ratio = (window.outerWidth || window.innerWidth) / (innerWidth || 1);
...
var ImageDithering = memo(function ImageDitheringImpl({
...
function TicketCard({
  ...
  const lines = splitName(name || "CREATOR");
  const qrCode = useMemo(() => {
    if (!qrValue) return null;
    try {
      return QRCode.create(qrValue, { errorCorrectionLevel: "M" });
    } catch {
      return null;
    }
  }, [qrValue]);
  ...
  return (
    ...
    >
      ...
      {/* Text layer */}
      <div className="absolute inset-0 pointer-events-none" style={{ color: layout.inkColor }}>
        {/* Presenter & Event */}
        <div
        ...
        {/* Footer: Venue & Dates */}
        <div
          className="absolute whitespace-nowrap uppercase font-semibold"
          style={{
            left: layout.padding * width,
        ...
        {qrCode && (
          ...
          >
            <div
              aria-label="Scan to view profile"
              className="grid w-full h-full"
              style={{
                gridTemplateColumns: `repeat(${qrCode.modules.size}, minmax(0, 1fr))`,
                gridTemplateRows: `repeat(${qrCode.modules.size}, minmax(0, 1fr))`
              }}
            >
...
function AdmitOneTicket({ tilt, width = REF, ...props }) {
  const geometry = props.geometry ?? TICKET_GEOMETRY;
  if (tilt === false) return <TicketCard width={width} {...props} />;
  return (
    <TiltCard
      clipPath={`path('${ticketClipPath(width, width / geometry.aspect, geometry)}')`}
      {...tilt}
    >
      <TicketCard width={width} {...props} />
    </TiltCard>
  );
}
